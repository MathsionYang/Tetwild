...
static bool show_app_log = true; // [Bruno] moved to global.
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref)
{
    ...
    if (ImGui::TreeNode("Settings"))
    {
        ...
        ImGui::SliderFloat("ScrollbarSize", &style.ScrollbarSize, 1.0f, 100.0f, "%.0f"); // [Bruno] increase max to 100.0f.
...
// [Bruno]
static ExampleAppLog the_log;
namespace ImGui {
    void debug_printf(const char* fmt, ...) {
	int old_size = the_log.Buf.size();
	va_list args;
	va_start(args, fmt);
	the_log.Buf.appendfv(fmt, args);
	va_end(args);
	for (int new_size = the_log.Buf.size(); old_size < new_size; old_size++)
	    if (the_log.Buf[old_size] == '\n')
		the_log.LineOffsets.push_back(old_size);
	the_log.ScrollToBottom = true;
    }
    void show_debug_console() {
	show_app_log = true;
    }
    void hide_debug_console() {
	show_app_log = false;
    }
...
static void ShowExampleAppLog(bool* p_open)
{
    ...
    // [Bruno] removed, I'm using this one for my own logging.
    /*
    ...
    if (time - last_time >= 0.20f && !ImGui::GetIO().KeyCtrl)
    {
        ...
        the_log.AddLog("[%s] Hello, time is %.1f, frame count is %d\n", random_words[rand() % IM_ARRAYSIZE(random_words)], time, ImGui::GetFrameCount());
        last_time = time;
    }
    */
    the_log.Draw("Example: Log", p_open); // [Bruno] using the_log global variable (used also by debug_printf).
}
