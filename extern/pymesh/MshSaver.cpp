...
void MshSaver::save_mesh_vtk(const VectorF &nodes, const VectorI &elements, size_t dim, ElementType type, Eigen::VectorXd &scalar)
{
    if (dim != 2 && dim != 3) {
        std::stringstream err_msg;
        err_msg << dim << "D mesh is not supported!" << std::endl;
    ...
    int elemSize = elements.size()/4, nodeSize = nodes.size()/m_dim;

    int i=0,j=0;
    char filename[256];

    std::string filestr=fname+".vtk";
    cout<<filestr<<endl;
    //filename=fname.c_str();

    FILE *fouts = fopen(filestr.c_str(),"w");
    fprintf(fouts,"# vtk DataFile Version 2.0\n");
    fprintf(fouts,"background mesh\n");
    fprintf(fouts,"ASCII\n");
    fprintf(fouts,"DATASET UNSTRUCTURED_GRID\n");
    fprintf(fouts,"POINTS %d double\n", nodeSize);


    for (i = 0; i < nodes.size(); i+=m_dim)
    {
        int index = i + 1;
        const VectorF& v = nodes.segment(i,m_dim);
        double x = v[0];
...
*/
    fprintf(fouts,"CELLS %d %d \n", elemSize, (elemSize) * 5);

    for (i = 0; i < elements.size(); i+=nodes_per_element)
    {
        ...
        fprintf(fouts,"4 %d %d %d %d\n",form0,form1,form2,form3);

    }
    fprintf(fouts,"CELL_TYPES %d \n", elemSize);
    ...
    {
        fprintf(fouts, "\nCELL_DATA %d\n", scalar.rows());
        fprintf(fouts, "SCALARS scalar float 1\n");
        fprintf(fouts, "LOOKUP_TABLE default\n");
